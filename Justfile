...
helm-check:
    ...
    helm template layerhouse deploy/kubernetes/helm --namespace layerhouse -f deploy/kubernetes/helm/test-values/minimal.yaml >/dev/null
    helm template layerhouse deploy/kubernetes/helm --namespace layerhouse -f deploy/kubernetes/helm/test-values/auth-enabled.yaml >/dev/null
    helm template layerhouse deploy/kubernetes/helm --namespace layerhouse -f deploy/kubernetes/helm/test-values/cert-manager.yaml >/dev/null
    helm template layerhouse deploy/kubernetes/helm --namespace layerhouse -f deploy/kubernetes/helm/test-values/air-gapped.yaml >/dev/null
    bash -euo pipefail -c 'out="$(mktemp)"; helm template layerhouse deploy/kubernetes/helm --namespace layerhouse -f deploy/kubernetes/helm/test-values/minimal.yaml > "$out"; grep -q "listen = \"0.0.0.0:5050\"" "$out"; grep -q "listen = \"0.0.0.0:5051\"" "$out"; rm -f "$out"'
    bash -euo pipefail -c 'out="$(mktemp)"; helm template layerhouse deploy/kubernetes/helm --namespace layerhouse -f deploy/kubernetes/helm/test-values/cert-manager.yaml > "$out"; grep -q "registry.example.internal" "$out"; grep -q "layerhouse-nodeport.example.internal" "$out"; rm -f "$out"'
    bash -euo pipefail -c 'out="$(mktemp)"; helm template layerhouse deploy/kubernetes/helm --namespace layerhouse -f deploy/kubernetes/helm/test-values/minimal.yaml --set replicaCount=1 > "$out"; grep -q "minAvailable: 1" "$out"; rm -f "$out"'
...
# Build layerhouse for the target platform and download RustFS + OxMgr,
...
# Build layerhouse-server for the target platform.
...
pack-build:
    ...
    @if which cargo-zigbuild > /dev/null 2>&1; then \
        ...
        cargo zigbuild --release -p layerhouse-server --target {{pack_target}}; \
    elif rustup target list --installed | grep -q {{pack_target}}; then \
        echo "Building with cargo for {{pack_target}}"; \
        cargo build --release -p layerhouse-server --target {{pack_target}}; \
    else \
    ...
    cp target/{{pack_target}}/release/layerhouse-server {{pack_out_dir}}/bin/
...
pack-tarball: pack-copy-configs pack-write-readme
    ...
    @tar -czf {{pack_out_dir}}/layerhouse-{{pack_version}}-{{pack_target}}.tar.gz \
        -C {{pack_out_dir}} bin config oxfile.toml README
    @echo "Tarball: {{pack_out_dir}}/layerhouse-{{pack_version}}-{{pack_target}}.tar.gz"
...
pack-write-readme:
    @echo 'layerhouse {{pack_version}} — binary deployment' > {{pack_out_dir}}/README
    @echo '' >> {{pack_out_dir}}/README
    @echo 'Quick start:' >> {{pack_out_dir}}/README
    @echo '  1. Start RustFS on port 9000' >> {{pack_out_dir}}/README
    @echo '  2. ./bin/layerhouse-server --config config/standalone.toml' >> {{pack_out_dir}}/README
    @echo '' >> {{pack_out_dir}}/README
...
pack-clean:
    ...
    rm -f {{pack_out_dir}}/layerhouse-*.tar.gz
...
tilt-up:
    ...
    tilt up --context kind-layerhouse-tilt

tilt-ci:
    tests/k8s/tilt/kind-up.sh
    tilt ci --context kind-layerhouse-tilt

tilt-ci-host-docker:
    tests/k8s/tilt/kind-up.sh
    tilt ci --context kind-layerhouse-tilt -- --host_docker_trust
