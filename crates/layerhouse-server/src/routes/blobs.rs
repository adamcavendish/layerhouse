...
async fn stat_or_pull<M: RegistryStore, B: BlobStore>(
        ...
        .await
    {
        Ok(true) => state
            ...
            .stat(digest)
            .await
            .map_err(|_| LayerhouseError::BlobUnknown(digest_str.to_string())),
        Ok(false) => Err(LayerhouseError::BlobUnknown(digest_str.to_string())),
        Err(e) => Err(e),
    }
}
...
async fn get_blob_range<M: RegistryStore, B: BlobStore>(
    ...
    let (start, end) = parse_range(range_value, total_size)
        .ok_or_else(|| LayerhouseError::Unsupported("invalid range".into()))?;
    ...
    let stream = state
        ...
        .await
        .map_err(|_| LayerhouseError::BlobUnknown(digest.to_string()))?;
