...
pub trait GcBlobStore: Send + Sync + 'static {
    async fn list_blob_objects(&self) -> Result<Vec<GcBlobObject>, LayerhouseError>;
    async fn delete_blob_batch(&self, keys: &[String]) -> Result<GcDeleteOutcome, LayerhouseError>;
}
...
impl GcBlobStore for S3GcBlobStore {
    ...
    async fn delete_blob_batch(&self, keys: &[String]) -> Result<GcDeleteOutcome, LayerhouseError> {
        ...
        let delete = Delete::builder()
            ...
            .build()
            .map_err(|e| LayerhouseError::Internal(e.to_string()))?;
...
async fn run_gc_sweep<B, F>(
    ...
    config: &GcConfig,
    mut still_leader: F,
) -> Result<GcStatus, LayerhouseError>
where
    B: GcBlobStore,
    F: FnMut() -> bool,
{
...
mod tests {
    ...
    impl GcBlobStore for FakeGcBlobStore {
        ...
        async fn delete_blob_batch(
            &self,
            keys: &[String],
        ) -> Result<GcDeleteOutcome, LayerhouseError> {
