...
impl S3BlobStore {
    ...
    async fn write_upload_state(
        ...
        let bytes = serde_json::to_vec(state)
            .map_err(|e| LayerhouseError::S3(format!("encode upload state: {e}")))?;
        self.client
            .put_object()
            .bucket(&self.bucket)
            ...
            .send()
            .await
            .map_err(|e| LayerhouseError::S3(e.to_string()))?;
        Ok(())
    }
    ...
    async fn read_upload_state(
        &self,
        session_id: &str,
    ) -> Result<Option<StagedUploadState>, LayerhouseError> {
        let result = self
            .client
            .get_object()
            .bucket(&self.bucket)
        ...
        let output = match result {
            ...
            Err(sdk_err) => {
                ...
                return Err(LayerhouseError::S3(service_err.to_string()));
            }
        };
        let bytes = output
            .body
        ...
        let state = serde_json::from_slice(&bytes)
            .map_err(|e| LayerhouseError::S3(format!("decode upload state: {e}")))?;
        Ok(Some(state))
    }

    async fn delete_staged_upload(&self, state: &StagedUploadState) -> Result<(), LayerhouseError> {
    ...
    async fn upload_final_part(
        ...
        part_number: i32,
        part_data: Bytes,
    ) -> Result<CompletedPart, LayerhouseError> {
        let resp = self
            .client
            .upload_part()
            .bucket(&self.bucket)
...
impl BlobStore for S3BlobStore {
    ...
    async fn stat(&self, digest: &Digest) -> Result<BlobInfo, LayerhouseError> {
        let resp = self
            ...
            .map_err(|e| {
                ...
                {
                    LayerhouseError::BlobUnknown(digest.to_string())
                } else {
                    LayerhouseError::S3(e.to_string())
                }
            })?;
    ...
    async fn push_chunk(&self, session_id: &str, data: Bytes) -> Result<u64, LayerhouseError> {
        let mut state = self
            ...
            .await?
            .ok_or_else(|| LayerhouseError::S3(format!("no upload session: {}", session_id)))?;
        ...
        for part in &state.parts {
            ...
            while buffer.len() >= MIN_PART_SIZE {
                if upload_id.is_none() {
                    ...
                    upload_id = Some(
                        resp.upload_id()
                            .ok_or_else(|| LayerhouseError::S3("no upload_id returned".into()))?
                            .to_string(),
                    );
                }
                final_part_number += 1;
        ...
        while let Some(chunk_result) = stream.next().await {
            let chunk = match chunk_result {
                ...
                Err(e) => {
                    ...
                    return Err(LayerhouseError::S3(e.to_string()));
                }
            };
            hasher.update(&chunk);
...
mod tests {
    ...
    async fn stat_unknown_blob_returns_blob_unknown_error() {
        ...
        assert!(matches!(result, Err(LayerhouseError::BlobUnknown(_))));
    }
