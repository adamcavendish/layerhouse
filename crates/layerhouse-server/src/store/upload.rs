...
impl UploadTracker {
    ...
    pub async fn get(&self, session_id: &str) -> Result<Option<UploadSession>, LayerhouseError> {
        match &self.backend {
            UploadTrackerBackend::Memory(sessions) => {
    ...
    pub async fn remove(&self, session_id: &str) -> Result<(), LayerhouseError> {
        match &self.backend {
            UploadTrackerBackend::Memory(sessions) => {
...
impl S3UploadTracker {
    ...
    async fn write(
        ...
        let bytes = serde_json::to_vec(session)
            .map_err(|e| LayerhouseError::S3(format!("encode upload session: {e}")))?;
        self.client
            .put_object()
            .bucket(&self.bucket)
    ...
    async fn read(&self, session_id: &str) -> Result<Option<UploadSession>, LayerhouseError> {
        let result = self
            .client
            .get_object()
            .bucket(&self.bucket)
        ...
        let session = serde_json::from_slice(&bytes)
            .map_err(|e| LayerhouseError::S3(format!("decode upload session: {e}")))?;
        Ok(Some(session))
    }

    async fn remove(&self, session_id: &str) -> Result<(), LayerhouseError> {
        self.client
            .delete_object()
            .bucket(&self.bucket)
            .key(Self::key(session_id))
