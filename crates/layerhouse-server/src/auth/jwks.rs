...
impl JwksCache {
    ...
    ) -> Result<usize, LayerhouseError> {
        let jwks: jsonwebtoken::jwk::JwkSet = serde_json::from_value(jwks_value.clone())
            .map_err(|e| LayerhouseError::Internal(format!("JWKS parse failed: {}", e)))?;

        let mut keys = Vec::new();
        for jwk in &jwks.keys {
            let kid = jwk.common.key_id.clone().unwrap_or_default();
            let decoding_key = jsonwebtoken::DecodingKey::from_jwk(jwk)
                .map_err(|e| LayerhouseError::Internal(format!("JWK conversion failed: {}", e)))?;
...
impl JwksS3Cache {
    ...
    pub async fn store(&self, document: &CachedJwksDocument) -> Result<(), LayerhouseError> {
        let bytes = serde_json::to_vec(document)
            .map_err(|e| LayerhouseError::Serialization(format!("JWKS cache encode: {e}")))?;
        self.client
            .put_object()
            .bucket(&self.bucket)
            .key(&self.key)
            ...
            .send()
            .await
            .map_err(|e| LayerhouseError::S3(format!("JWKS cache upload failed: {e}")))?;
        tracing::info!(key = %self.key, "stored last-good JWKS cache in S3");
        Ok(())
    }

    pub async fn load(&self) -> Result<Option<CachedJwksDocument>, LayerhouseError> {
        let result = self
            .client
            .get_object()
            .bucket(&self.bucket)
        ...
        let output = match result {
            ...
            Err(sdk_err) => {
                ...
                return Err(LayerhouseError::S3(format!(
        ...
        let bytes = output
            ...
            .collect()
            .await
            .map_err(|e| LayerhouseError::S3(format!("JWKS cache body read failed: {e}")))?
            .to_vec();
        let document = serde_json::from_slice(&bytes)
            .map_err(|e| LayerhouseError::Serialization(format!("JWKS cache decode: {e}")))?;
        Ok(Some(document))
    }
}
...
pub async fn fetch_jwks(
    ...
    let response = client
        ...
        .map_err(|e| LayerhouseError::Internal(format!("JWKS request build failed: {}", e)))?
        .send()
        .await
        .map_err(|e| LayerhouseError::Internal(format!("JWKS fetch failed: {}", e)))?;

    let body = response
        .text()
        .await
        .map_err(|e| LayerhouseError::Internal(format!("JWKS read failed: {}", e)))?;
