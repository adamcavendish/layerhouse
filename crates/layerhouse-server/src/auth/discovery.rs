...
impl OidcDiscovery {
    ...
    pub(crate) async fn fetch_document(
        issuer_url: &str,
        tls_insecure: bool,
    ) -> Result<serde_json::Value, LayerhouseError> {
        ...
        let response = client
            ...
            .map_err(|e| {
                LayerhouseError::Internal(format!("discovery request build failed: {}", e))
            })?
            .send()
            .await
            .map_err(|e| LayerhouseError::Internal(format!("discovery fetch failed: {}", e)))?;

        let body = response
            .text()
            .await
            .map_err(|e| LayerhouseError::Internal(format!("discovery read failed: {}", e)))?;

        serde_json::from_str(&body)
            .map_err(|e| LayerhouseError::Internal(format!("discovery parse failed: {}", e)))
    }
    ...
    pub(crate) fn from_document(
        ...
        let authorization_endpoint = doc["authorization_endpoint"]
            .as_str()
            .ok_or_else(|| {
                LayerhouseError::Internal("discovery missing authorization_endpoint".to_string())
            })?
            .to_string();
        ...
        let token_endpoint = rewrite_base(
            doc["token_endpoint"].as_str().ok_or_else(|| {
                LayerhouseError::Internal("discovery missing token_endpoint".to_string())
            })?,
            public_base,
            issuer_url,
        );
        ...
        let jwks_uri = rewrite_base(
            doc["jwks_uri"].as_str().ok_or_else(|| {
                LayerhouseError::Internal("discovery missing jwks_uri".to_string())
            })?,
            public_base,
            issuer_url,
        );
...
/// "https://localhost:8443/oauth2/openid/layerhouse" → "https://localhost:8443"
...
mod tests {
    ...
    fn extracts_origin_from_full_issuer_url() {
        assert_eq!(
            extract_origin("https://localhost:8443/oauth2/openid/layerhouse"),
            "https://localhost:8443"
        );
        assert_eq!(
            extract_origin("https://idp.internal:8443/oauth2/openid/layerhouse"),
    ...
    fn rewrites_public_origin_to_internal() {
        assert_eq!(
            rewrite_base(
                ...
                "https://localhost:8443/oauth2/openid/layerhouse",
                "https://idp.internal:8443/oauth2/openid/layerhouse"
            ),
            "https://idp.internal:8443/oauth2/token"
        );
        assert_eq!(
            rewrite_base(
                "https://localhost:8443/oauth2/openid/layerhouse/public_key.jwk",
                "https://localhost:8443/oauth2/openid/layerhouse",
                "https://idp.internal:8443/oauth2/openid/layerhouse"
            ),
