...
layerhouse uses [OpenRaft](https://github.com/datafuselabs/openraft) for distributed
...
- **Node ID from hostname.** Container hostname must be `<prefix>-<N>`. Node ID is
  `N + 1` (so `layerhouse-0` becomes node 1).
...
```mermaid
sequenceDiagram
    participant N1 as layerhouse-0
    participant DNS as DNS
    participant N2 as layerhouse-1
    participant N3 as layerhouse-2

    N1->>DNS: resolve layerhouse
    DNS-->>N1: no peers
    N1->>N1: bootstrap (node 1)

    N2->>DNS: resolve layerhouse
    DNS-->>N2: layerhouse-0
    N2->>N1: POST /raft/join
    N1-->>N2: membership updated

    N3->>DNS: resolve layerhouse
    DNS-->>N3: layerhouse-0, layerhouse-1
...
- Docker Compose: `hostname: layerhouse-0`
- Kubernetes: StatefulSet pod names (`layerhouse-0`, `layerhouse-1`)
