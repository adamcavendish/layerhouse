# layerhouse

- [Introduction](introduction.md)
- [Quick Start](quickstart.md)

- [Architecture](architecture.md)
  - [Data Flow](architecture/data-flow.md)
  - [Raft Clustering](architecture/raft.md)
  - [S3 Blob Storage](architecture/s3.md)

- [Configuration](configuration.md)
  - [Server](configuration/server.md)
  - [Storage](configuration/storage.md)
  - [Raft](configuration/raft.md)
  - [Authentication](configuration/auth.md)

- [Deployment](deployment/index.md)
  - [Kubernetes](deployment/kubernetes.md)
  - [Docker Compose](deployment/docker-compose.md)
  - [Binary](deployment/binary.md)

- [Clustering](clustering.md)
  - [DNS Discovery](clustering/dns-discovery.md)
  - [Snapshot & Recovery](clustering/snapshots.md)
  - [Air-Gapped Kubernetes](clustering/air-gapped-kubernetes.md)

- [Authentication](authentication.md)
  - [Kanidm Setup](authentication/kanidm.md)
  - [Bring Your Own IdP](authentication/bring-your-own-idp.md)
  - [Personal Access Tokens](authentication/pat.md)
  - [CI / Service Accounts](authentication/service-accounts.md)
  - [Dashboard OIDC](authentication/oidc.md)

- [Mirror & Proxy Cache](mirror-proxy.md)
- [Dashboard](dashboard.md)

- [Operations](operations.md)
  - [Health Checks](operations/health.md)
  - [Garbage Collection](operations/gc.md)
  - [Monitoring](operations/monitoring.md)
  - [TLS Rotation](operations/tls-rotation.md)
  - [Upgrade Limitations](operations/upgrade-limitations.md)

- [Reference](reference/api.md)
  - [Configuration Reference](reference/config-reference.md)
  - [OCI Distribution Spec](reference/oci-spec.md)
